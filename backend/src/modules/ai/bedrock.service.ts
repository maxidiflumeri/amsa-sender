...
export class BedrockService {
  ...
  constructor(private readonly config: ConfigService) {
    this.client = new BedrockRuntimeClient({
      region: this.config.get<string>('AWS_BEDROCK_REGION') ?? 'us-east-1',
      credentials: {
        accessKeyId: this.config.get<string>('AWS_BEDROCK_ACCESS_KEY_ID')!,
        secretAccessKey: this.config.get<string>('AWS_BEDROCK_SECRET_ACCESS_KEY')!,
      },
    });
    this.modelId = this.config.get<string>('AWS_BEDROCK_MODEL_ID') ?? 'anthropic.claude-3-haiku-20240307-v1:0';
  }

  private async invoke(prompt: string, temperature = 0.7): Promise<string> {
    const llama3Prompt =
...
Sugerí una respuesta profesional y cordial para el agente basándote en la conversación y la base de conocimiento. Devolvé SOLO el texto de la respuesta, sin comillas, sin explicaciones, sin prefijos.`;
  ...
  async generarAnalisisCampania(metrics: any, historial: any[] = []): Promise<string> {
    ...
    const prompt = `Analizá los siguientes datos de una campaña de WhatsApp y generá un informe completo y detallado en español argentino. Cada sección debe tener contenido sustancial — no uses frases genéricas, basate siempre en los datos concretos provistos.
...
Usá este historial para comparar el desempeño actual vs campañas anteriores del mismo template.` : '\n(Sin campañas anteriores del mismo template para comparar)'}
...
⚠️ ALERTAS
• [alerta 1 si aplica, o "ninguna" si todo está bien]
...
2. [hasta 4 recomendaciones]`;
    ...
    try {
      return await this.invoke(prompt, 0);
    } catch (err: any) {
  ...
  async generarAnalisisAgentes(metrics: any): Promise<string> {
    ...
    const mejorRespuesta = [...agentes].filter(a => a.avgPrimeraRespuestaMs).sort((a, b) => a.avgPrimeraRespuestaMs - b.avgPrimeraRespuestaMs)[0];
    const peorRespuesta = [...agentes].filter(a => a.avgPrimeraRespuestaMs).sort((a, b) => b.avgPrimeraRespuestaMs - a.avgPrimeraRespuestaMs)[0];
    ...
    const horaGlobalPico = agentes
      ...
      .reduce((acc: Record<number, number>, h: any) => { acc[h.hora] = (acc[h.hora] ?? 0) + h.count; return acc; }, {});
    ...
    const prompt = `Analizá el desempeño del equipo de agentes de WhatsApp y generá un informe completo y detallado en español argentino. Cada sección debe tener contenido sustancial — no uses frases genéricas, basate siempre en los datos concretos provistos.
...
🔴 ATENCIÓN
• [agentes que necesitan atención — con datos, o "ninguno" si todo está bien]

📈 PATRONES
• [patrones de actividad, horas, tendencias]
